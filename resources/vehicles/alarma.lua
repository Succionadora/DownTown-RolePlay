...
function onVehicleDamage(ignoredElement)
	...
	if (not driver or driver == ignoredElement) and (not passenger1 or passenger1 == ignoredElement) and (not passenger2 or passenger2 == ignoredElement) and (not passenger3 or passenger3 == ignoredElement) then
		if getElementData(source, "havealarm") == true then
...
function stopAlarm (vehicle)
	if vehicle then
		for k, v in ipairs(getElementsByType("player")) do
			triggerClientEvent(v, "stopCarAlarm", v, vehicle)
		end
	end
end
addEvent("onStopAlarm", true)
addEventHandler("onStopAlarm", getRootElement(), stopAlarm)