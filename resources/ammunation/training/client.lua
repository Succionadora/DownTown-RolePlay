...
addEventHandler ( "NGAmmunation:Module->Training:onClientHitMarker", root, function ( s )
	...
	if not ( d[allowedWeapons[w]] ) then 
		return outputChatBox ( "This weapon isn't allowed to be trained.", 255, 255, 0 )
	end
	if ( d[allowedWeapons[w]] > 90 ) then
		return outputChatBox ( "This weapon has maximum efficiency!", 255, 255, 0 )
	end
	...
	if ( k == "space" ) then
		...
		elseif ( selectedOption == 0 ) then
			...
			if ( not allowedWeapons [ getPedWeapon ( localPlayer ) ] ) then
				return outputChatBox ( "This weapon isn't allowed" )
			end
			
			if ( getPlayerMoney ( ) < 1200 ) then
				return outputChatBox ( "You don't have $1,200 to train this weapon", 255, 255, 0 )
			end
		...
		if ( training.TargetShot == training.RequiredPeds ) then
			...
			setTimer ( function ( )
				...
				if ( trainingWeapon ~= getPedWeapon ( localPlayer ) ) then
					return outputChatBox ( "You have changed your weapon.... No skills added.", 255, 0, 0 )
				end
				...
				outputChatBox ( "Weapon ranked up! New level: "..d[allowedWeapons[w]].."/100", 0, 255, 0 )
