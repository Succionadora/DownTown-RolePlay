-- Se ha realizado una edición del mapa 18/10/2017
...
function toggleCustomTiles ( )
	...
	timer = setTimer ( handleTileLoading, 2000, 0 )
end
addEventHandler("onClientResourceStart", getRootElement(), toggleCustomTiles)

function toggleRadar()
	if getElementData(getLocalPlayer(), "radar") == false or getElementData(getLocalPlayer(), "radar") == nil then
		setElementData(getLocalPlayer(), "radar", true)
	else
		setElementData(getLocalPlayer(), "radar", false)
	end
end	
addCommandHandler("minimapa", toggleRadar)  

function handleTileLoading ( )
	-- Get all visible radar textures
	if getElementData(getLocalPlayer(), "radar") == true then return end
	...
	for name, data in pairs ( tiles ) do
		if not table.find ( visibleTileNames, name ) or getElementData(getLocalPlayer(), "radar") == true then
			unloadTile ( name )
		end
	end
...
function loadTile ( name )
	...
	-- Fetch the filename
	local file = string.format ( "radar/radar_%d_%d.png", row, col )
