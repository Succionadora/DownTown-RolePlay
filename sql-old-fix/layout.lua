			...
			if not fields[ value.name ] then
				...
				if getColumnString( f ) ~= str then
					if query_update( "ALTER TABLE `%s` MODIFY COLUMN " .. str, name ) then
						...
						outputDebugString( "Error GRAVE OMITIDO - Es normal si la versión de MySQL es antigua" )
						outputServerLog( "Error GRAVE OMITIDO - Es normal si la versión de MySQL es antigua" )
						--return false
					end
				end
